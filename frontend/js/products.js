import { fetchProducts, specialSearchProducts, keyFilterProducts, searchProducts, filterProducts, API_BASE_URL } from "./utils.js";
...
function productBuilder(elementID, searchFilter) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pageProducts.forEach((product) => {
            ...
            // Use the correct image path from API or fallback to default
            ...
            if (product.discount > 0) {
                ...
                const discountedPrice = product.finalPrice || (originalPrice * (1 - product.discount / 100));
                productPrice.innerHTML = `<span>$${discountedPrice.toFixed(2)}</span> <span class="text-decoration-line-through text-muted">$${originalPrice.toFixed(2)}</span>`;
            } else {
