...
function ProductBodyBuilder(product) {
    ...
    if (product.rating % 1 != 0) {
        ...
        div.appendChild(i);
    } div.innerHTML += `<span class="text-muted"> ${product.rating.toFixed(1)}/5</span>`;
...
function addReviewsSection(container) {
    ...
    reviewsRow.appendChild(reviewsCol);    // Add a heading for the reviews section
    ...
    if (product.reviews && product.reviews.length > 0) {
        ...
        product.reviews.forEach(review => {
            ...
            ratingText.textContent = `${parseFloat(review.rating).toFixed(1)}/5`;
